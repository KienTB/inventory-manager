<div>
    <div class="card pos-panel" wire:poll.2s>
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('pos.checkout') }}" class="mt-3" id="checkout-form" 
                  onsubmit="event.preventDefault(); handleCheckout(event);">
                @csrf
                <input type="hidden" name="customer_id" value="{{ $customer_id }}" />
                <input type="hidden" name="cart_instance" value="{{ $cartInstance }}" />
                <input type="hidden" name="payment_type" id="payment_type" value="" />
                <input type="hidden" name="pay" id="pay_amount" value="{{ $this->received > 0 ? $this->received : $this->total }}" />
                <button class="btn btn-primary w-100" type="submit" id="checkout-btn">
            ...
            <script>
                ...
                Livewire.on('cartUpdated', () => {
                    const received = @js($this->received);
                    const total = @js($this->total);
                    document.getElementById('pay_amount').value = received > 0 ? received : total;
                });
            </script>
        </div>
    </div>
